...
public:
   ...
   *
   * @param[in]  resolution  The resolution  指定分辨率
  ...
  bool getImages(cv::Mat& left_image, cv::Mat& right_image)
  {
    ...
    if (camera_->grab())
    {
      ...
      cv::Rect left_rect(0, 0, width_ / 2, height_);  // x, y, width, height
...
private:
  ...
  int height_;
  double frame_rate_;
  bool cv_three_;  // flag to check if the opencv version is 3.x
};
...
public:
  ...
  ZedCameraROS()
  {
    ...
    correctFramerate(resolution_, frame_rate_);  // correct frame rate according to resolution
    ...
    if (use_zed_config_)
    {
      ...
      if (!config_file_location_.empty())
      {
        try
        {
          getZedCameraInfo(config_file_location_, resolution_, left_info, right_info);  // get camera info from zed
        }
    ...
    ros::Rate r(frame_rate_);  // set the frame rate
  ...
  {
    cam_info_msg.header.stamp = now;  // update the time stamp
